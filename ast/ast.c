...
void freeFunc(astNode *node) {
    ...
    free(node->func.name);
    if (node->func.param != NULL) freeVar(node->func.param);
...
void freeCall(astNode *node) {
    ...
    if (node->stmt.call.param != NULL) freeNode(node->stmt.call.param);
...
void freeIf(astNode *node) {
    ...
    if (node->stmt.ifn.else_body != NULL) freeNode(node->stmt.ifn.else_body);
...
void printNode(astNode *node, int n) {
    ...
    switch (node->type) {
        ...
        case ast_func: {
            ...
            if (node->func.param != NULL) printNode(node->func.param, n + 1);
