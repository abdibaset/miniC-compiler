/** @author: Vasanta Lakshmi Kommineni */
#ifndef AST_H
#define AST_H
...
typedef struct {
    ...
    astNode*
        param;  // parameter, possibly NULL if the function doesn't take a param
    astNode* body;  // function body
} astFunc;

typedef struct {
    char*
        name;  // For extern functions defined we will only save function names
} astExtern;
...
/*
Declarations for all free* functions. All these functions take a astNode* as
parameter as free the memory allocated by corresponding create functions.
*/
...
/* Function to print astNode and astStmt. The second parameter is to beautify
 * the output.*/
