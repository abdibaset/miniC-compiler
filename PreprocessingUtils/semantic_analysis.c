...
/**
 * @details this is  helper method that checks if a variable has been declared
 * from top of stack to bottom
 *
 * @return 1 - variable has been declared in a valid scope - current scope or
 * global scope
 * @return 0 - variable not declared in valid scope/ referenced before
 * declaration
 *
 */
int isVariableDeclared(astNode *node) {
    ...
    for (int stackIndex = stack.size() - 1; stackIndex >= 0; stackIndex--) {
        ...
        for (int symTableIndex = 0; symTableIndex < (int)symTable.size();
             symTableIndex++) {
        ...
        case ast_decl:
            ...
            while (it != curr_symbol_table.end()) {
                if (strcmp(*it, statement->decl.name) == 0) {
                    fprintf(stderr,
                            "Variable %s has been declared more than once in "
                            "this scope\n",
                            statement->decl.name);
    ...
    switch (node->type) {
        ...
        case ast_stmt:
            ...
            statement_evaluation(
                statement, isFunction);  // check the statement type and content
            break;
        ...
        case ast_var:
            if (!isVariableDeclared(node)) {
                fprintf(stderr,
                        "Variable %s referenced before declaration in valid "
                        "scope\n",
                        node->var.name);
                IS_REFERENCED_B4_DECL =
                    true;  // if variable is being referenced before declared in
                           // a valid scope
            }
            break;
