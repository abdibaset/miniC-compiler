...
int localMem;
LLVMValueRef PARAM;
void find_spill_and_reallocate_registers(LLVMValueRef currInstruction,
...
bool is_operand_live(LLVMValueRef currInstruction,
                     LLVMValueRef instructionToCheck) {
    return CURRENT_LIVENESS_MAP[instructionToCheck].back() >
           INST_INDEX_MAP[currInstruction];
}

void handle_instruction_without_return(LLVMValueRef currInstruction,
                                       vector<int> &registers) {
    ...
    for (const auto &operand : operands) {
        if (CURRENT_LIVENESS_MAP.find(operand) != CURRENT_LIVENESS_MAP.end()) {
            bool isOperandLive = INST_INDEX_MAP[currInstruction] <
                                 CURRENT_LIVENESS_MAP[operand].back();
...
void handle_binary_operation_instruction(LLVMValueRef currInstruction,
                                         vector<int> &registers) {
    // get operands of intruction
    vector<LLVMValueRef> operands = getOperands(currInstruction);

    // check first operand in reg map and has register
    bool isFirstOperandAssignedRegister =
        INST_REG_MAP.find(operands[0]) != INST_REG_MAP.end() &&
        INST_REG_MAP[operands[0]] != -1;
    bool isSecondOperandAssignedRegister =
        INST_REG_MAP.find(operands[1]) != INST_REG_MAP.end() &&
        INST_REG_MAP[operands[1]] != -1;
    bool isFirstOperandLastSeen =
        CURRENT_LIVENESS_MAP.find(operands[0]) != CURRENT_LIVENESS_MAP.end() &&
        CURRENT_LIVENESS_MAP[operands[0]][1] == INST_INDEX_MAP[currInstruction];
    bool isSecondOperandLive =
        CURRENT_LIVENESS_MAP.find(operands[1]) != CURRENT_LIVENESS_MAP.end() &&
        is_operand_live(currInstruction, operands[1]);
...
void find_spill_and_reallocate_registers(LLVMValueRef currInstruction,
    ...
    int currInstLivenessRange = CURRENT_LIVENESS_MAP[currInstruction].back() -
                                CURRENT_LIVENESS_MAP[currInstruction][0];

    // Assign the highest range instruction's register to currInstruction if its
    // range is lower
    if (currInstLivenessRange < highestRange) {
        INST_REG_MAP[currInstruction] =
            INST_REG_MAP[instructionWithHighestRange];
...
void handle_other_instructions(LLVMValueRef currInstruction,
                               vector<int> &registers) {
    vector<LLVMValueRef> operands = getOperands(currInstruction);
    for (const auto &operand : operands) {
        bool isOperandInRegMap =
            INST_REG_MAP.find(operand) != INST_REG_MAP.end();
        if (isOperandInRegMap && INST_REG_MAP[operand] != -1 &&
            !is_operand_live(currInstruction, operand)) {
            registers.push_back(INST_REG_MAP[operand]);
        }
    }
}

void check_operands_to_update_reg_map(LLVMValueRef instruction,
...
void allocate_registers(LLVMBasicBlockRef basicBlock) {
    ...
    for (LLVMValueRef instruction = LLVMGetFirstInstruction(basicBlock);
        ...
        // If Instr is an instruction that does not have a result (e.g. store,
        // branch, call that doesn't return a value):
        LLVMOpcode opcode = getOpcode(instruction);
        if (opcode != LLVMAlloca) {
            ...
            if (INST_REG_MAP.find(instruction) == INST_REG_MAP.end() &&
                LLVMGetTypeKind(LLVMTypeOf(instruction)) != LLVMVoidTypeKind) {
                // available resource
                if (!registers.empty()) {
...
void compute_liveness_and_map_instruction_to_index(
    ...
    for (LLVMValueRef instruction = LLVMGetFirstInstruction(basicBlock);
        ...
        if (opcode != LLVMAlloca) {
            if ((opcode == LLVMAdd || opcode == LLVMSub || opcode == LLVMMul ||
                 opcode == LLVMICmp) ||
                (opcode == LLVMCall && LLVMGetTypeKind(LLVMTypeOf(
                                           instruction)) != LLVMVoidTypeKind)) {
...
void print_directives(bool overWrite = false,
                      const char *instructionToWriteToFile = NULL) {
    FILE *file;
    if (overWrite) {
...
void gen_load_instruction_assembly_code(LLVMValueRef instruction,
    ...
    if (INST_REG_MAP[instruction] != -1) {
        ...
        string directiveBuilder = "\tmovl\t" + to_string(operandOffsetVal) +
                                  "(%ebp),\t%" +
                                  REG_NAME[INST_REG_MAP[instruction]] + "\n";
        print_directives(false, directiveBuilder.c_str());
    }
}

void gen_return_instruction_assembly_code(LLVMValueRef instruction,
                                          map<LLVMValueRef, int> offsetMap) {
    print_directives(false, get_instruction_string(instruction).c_str());
    vector<LLVMValueRef> operands = getOperands(instruction);
    string directive;
    if (LLVMIsConstant(operands[0])) {
        ...
        directive =
            "\tmovl\t" + to_string(offsetOfTempVariable) + "(%ebp),\t%eax\n";
    } else if (INST_REG_MAP[operands[0]] != -1) {
        directive =
            "\tmovl\t%" + REG_NAME[INST_REG_MAP[operands[0]]] + ",\t%eax\n";
    }
    directive += "\tpopl\t%ebx\n";
...
void gen_store_instruction_assembly_code(LLVMValueRef instruction,
                                         map<LLVMValueRef, int> offsetMap) {
    vector<LLVMValueRef> operands = getOperands(instruction);
    ...
    if (operands[0] != PARAM) {
        ...
        if (LLVMIsConstant(operands[0])) {
            ...
            long long constVal = LLVMConstIntGetSExtValue(operands[0]);
            directiveBuilder = "\tmovl\t$" + to_string(constVal) + ",\t" +
                               to_string(secondOperandOffset) + "(%ebp)\n";
            print_directives(false, directiveBuilder.c_str());
        } else {
            ...
            if (INST_REG_MAP[operands[0]] != -1) {
                directiveBuilder =
                    "\tmovl\t%" + REG_NAME[INST_REG_MAP[operands[0]]] + ",\t" +
                    to_string(offsetOfSecondOperand) + "(%ebp)\n";
                print_directives(false, directiveBuilder.c_str());
            } else {
                int offsetOfFirstOperand = offsetMap[operands[0]];
                string firstDirectiveBuilder =
                    "\tmovl\t" + to_string(offsetOfFirstOperand) + ",\t%eax\n";
                print_directives(false, firstDirectiveBuilder.c_str());

                string secondDirectiveBuilder =
                    "\tmovl\t%eax," + to_string(offsetOfSecondOperand) +
                    "\t(%ebp)\n";
...
void gen_call_instruction_assembly_code(LLVMValueRef instruction,
                                        map<LLVMValueRef, int> offsetMap) {
    print_directives(false, get_instruction_string(instruction).c_str());
    bool hasParam = false;
    ...
    if (operands.size() > 1) {
        ...
        if (LLVMIsConstant(operands[0])) {
            ...
            if (INST_REG_MAP[operands[0]] != -1) {
                string directiveBuilder =
                    "\tpushl\t%" + REG_NAME[INST_REG_MAP[operands[0]]] + "\n";
                print_directives(false, directiveBuilder.c_str());
            } else if (INST_REG_MAP[operands[0]] == -1) {
                int offsetOfParam = offsetMap[operands[0]];
                string directiveBuilder =
                    "\tpushl\t" + to_string(offsetOfParam) + "(%ebp)\n";
                print_directives(false, directiveBuilder.c_str());
            }
        }
    }
    size_t length;
    const char *functioName =
        LLVMGetValueName2(getOperands(instruction).back(), &length);
    ...
    if (LLVMGetTypeKind(LLVMTypeOf(instruction)) != LLVMVoidTypeKind) {
        ...
        if (INST_REG_MAP[instruction] != -1) {
            directiveBuilder3 = "\tmovl\t%eax,\t%" +
                                REG_NAME[INST_REG_MAP[instruction]] + "\n ";
        } else {
            int offsetOfCurrInstruction = offsetMap[instruction];
            directiveBuilder3 = "\tmovl\t%eax,\t" +
                                to_string(offsetOfCurrInstruction) + "(%ebp)\n";
        }
    }
...
void gen_branch_instruction_assembly_code(LLVMValueRef instruction,
                                          map<LLVMValueRef, int> offsetMap) {
    print_directives(false, get_instruction_string(instruction).c_str());
...
void gen_binary_instruction_assembly_code(LLVMValueRef instruction,
                                          map<LLVMValueRef, int> offsetMap) {
    print_directives(false, get_instruction_string(instruction).c_str());
    string registerVal;
    if (INST_REG_MAP[instruction] != -1) {
    ...
    if (LLVMIsConstant(operands[0])) {
        long long constVal = LLVMConstIntGetSExtValue(operands[0]);
        firstOperandDirectiveBuilder =
            "\tmovl$" + to_string(constVal) + ",\t" + registerVal + "\n";
    } else {
        if (INST_REG_MAP[operands[0]] != -1 &&
            strcmp(REG_NAME[INST_REG_MAP[operands[0]]].c_str(),
                   registerVal.c_str()) != 0) {
            firstOperandDirectiveBuilder = "\tmovl\t%" +
                                           REG_NAME[INST_REG_MAP[operands[0]]] +
                                           ",\t " + registerVal + "\n ";
        } else {
            int offsetVal = offsetMap[operands[0]];
            firstOperandDirectiveBuilder = "\tmovl\t" + to_string(offsetVal) +
                                           "(%ebp),\t" + registerVal + "\n";
        }
    }
    ...
    if (LLVMIsConstant(operands[1])) {
        ...
        secondOperandDirectiveBuilder = "\t" + arthimeticType + "\t$" +
                                        to_string(constVal) + ",\t" +
                                        registerVal + "\n";
    } else {
        if (INST_REG_MAP[operands[1]] != -1) {
            secondOperandDirectiveBuilder =
                "\t" + arthimeticType + "\t%" +
                REG_NAME[INST_REG_MAP[operands[1]]] + ",\t" + registerVal +
                "\n";
        } else {
            int offsetVal = offsetMap[operands[1]];
            secondOperandDirectiveBuilder = "\t" + arthimeticType + "\t" +
                                            to_string(offsetVal) + "(%ebp),\t" +
                                            registerVal + "\n";
        }
    }
    ...
    if (INST_REG_MAP[instruction] != -1) {
        ...
        string instDirectiveBuilder =
            "\tmovl\t%eax,\t" + to_string(offsetVal) + "(%ebp)\n";
        print_directives(false, instDirectiveBuilder.c_str());
    }
}

void gen_compare_instruction_assembly_code(LLVMValueRef instruction,
                                           map<LLVMValueRef, int> offsetMap) {
    print_directives(false, get_instruction_string(instruction).c_str());
    string registerVal;
    if (INST_REG_MAP[instruction] != -1) {
    ...
    // first operand
    if (LLVMIsConstant(operands[0])) {
        firstOperandDirective =
            "\tmovl\t$" + to_string(LLVMConstIntGetSExtValue(operands[0])) +
            ",\t" + registerVal + "\n";
    } else {
        if (INST_REG_MAP[operands[0]] != -1 &&
            strcmp(REG_NAME[INST_REG_MAP[operands[0]]].c_str(),
                   registerVal.c_str()) != 0) {
            firstOperandDirective = "\tmovl\t%" +
                                    REG_NAME[INST_REG_MAP[operands[0]]] +
                                    ",\t " + registerVal + "\n ";
        } else {
            int offsetVal = offsetMap[operands[0]];
            firstOperandDirective = "\tmovl\t" + to_string(offsetVal) +
                                    "(%ebp),\t" + registerVal + "\n";
        }
    }
    ...
    if (LLVMIsConstant(operands[1])) {
        secondOperandDirective =
            "\tcmpl\t$" + to_string(LLVMConstIntGetSExtValue(operands[1])) +
            ",\t" + registerVal + "\n";
    } else {
        if (INST_REG_MAP[operands[1]] != -1) {
            secondOperandDirective = "\tcmpl\t%" +
                                     REG_NAME[INST_REG_MAP[operands[1]]] +
                                     ",\t" + registerVal + "\n";
        } else {
            int offsetVal = offsetMap[operands[1]];
            secondOperandDirective = "\tcmpl\t" + to_string(offsetVal) +
                                     "(%ebp),\t" + registerVal + "\n";
        }
    }
...
void gen_assembly_code_for_basic_block(LLVMBasicBlockRef basicBlock,
                                       map<LLVMValueRef, int> offsetMap) {
    for (LLVMValueRef instruction = LLVMGetFirstInstruction(basicBlock);
         instruction; instruction = LLVMGetNextInstruction(instruction)) {
        LLVMOpcode opcode = getOpcode(instruction);
...
void walk_basic_blocks_for_register_allocation_and_codegen(
    LLVMValueRef function, map<LLVMValueRef, int> offsetMap) {
...
void walk_functions_for_register_allocation_and_codegen(
    LLVMModuleRef moduleReference, map<LLVMValueRef, int> offsetMap) {
    for (LLVMValueRef function = LLVMGetFirstFunction(moduleReference);
         function; function = LLVMGetNextFunction(function)) {
        size_t length;
        const char *functionName = LLVMGetValueName2(function, &length);
        if (strcmp(functionName, "main-func") == 0) {
            string directives = "\t.text\n\t.globl\n\t.type\n\tfunction " +
                                string(functionName) + "\nfunc:\n";
            string emitInstructions =
                "\tpushl\t%ebp\n\tmovl\t%esp,\t%ebp\n\tsubl\t$" +
                to_string(localMem - 4) + ",\t%esp\n\tpushl\t%ebx\n";
        ...
        walk_basic_blocks_for_register_allocation_and_codegen(function,
                                                              offsetMap);
    }
}
...
map<LLVMValueRef, int> get_offset_map(LLVMModuleRef moduleReference) {
    ...
    for (LLVMValueRef function = LLVMGetFirstFunction(moduleReference);
         function; function = LLVMGetNextFunction(function)) {
        // add the LLVMValueRef of the parameter as key and its associated value
        // as 8 to the offset_map.
        for (size_t i = 0; i < LLVMCountParams(function); i++) {
            functionParam =
                LLVMGetParam(function, i);  // guaranteed single param
        ...
        for (LLVMBasicBlockRef basicBlock = LLVMGetFirstBasicBlock(function);
            ...
            for (LLVMValueRef instruction = LLVMGetFirstInstruction(basicBlock);
                ...
                switch (opcode) {
                    ...
                    case LLVMStore: {
                        // If the first operand of the store instruction is
                        // equal to the function parameter
                        vector<LLVMValueRef> operands =
                            getOperands(instruction);
                        bool isFirstOperandFuncParam =
                            operands[0] == functionParam;
                        if (isFirstOperandFuncParam) {
                            // Get the value associated with the first operand
                            // in offset_map. Let this be x.
                            int firstOperandOffsetFromMap =
                                offsetMap[operands[0]];
                            // Change the value associated with the second
                            // operand in  offset_map to x
                            offsetMap[operands[1]] = firstOperandOffsetFromMap;
                        } else {  // If the first operand of the store
                                  // instruction is not equal to the function
                                  // parameter
                            int secondOperandOffsetFromMap =
                                offsetMap[operands[1]];
                            offsetMap[operands[0]] = secondOperandOffsetFromMap;
                        }
                    } break;

                    case LLVMLoad: {
                        // Get the value associated with the first operand in
                        // offset_map. Let this be x.
                        vector<LLVMValueRef> operands =
                            getOperands(instruction);
...
string generateOutputFilename(const string &inputPath) {
    ...
    std::string baseName =
        (lastDot == std::string::npos) ? filename : filename.substr(0, lastDot);
...
void generate_assembly_code(const char *filename,
    ...
    walk_functions_for_register_allocation_and_codegen(moduleReference,
                                                       offsetMap);
}
