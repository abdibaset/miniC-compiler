...
map<LLVMBasicBlockRef, set<LLVMValueRef>> BLOCK_TO_ALL_STOREINSTRUCTIONS,
    BLOCK_TO_OUTSET, BLOCK_TO_INSET;
...
void replaceValidLoadInstructionsOperandsWithConstant(
    set<LLVMValueRef> inSet, LLVMValueRef loadInstruction) {
    ...
    if (constVal != NULL) {
        LLVMValueRef constValue =
            LLVMConstInt(LLVMTypeOf(loadInstruction),
                         LLVMConstIntGetSExtValue(constVal), false);
        LLVMReplaceAllUsesWith(loadInstruction, constValue);
        LOAD_INSTRUCTIONS_TO_REPLACE.push_back(loadInstruction);
    }
}

set<LLVMValueRef> removeKilledStoreInstruction(
    set<LLVMValueRef> inSet, LLVMValueRef currentStoreInstruction) {
    LLVMValueRef currInstMemoryOperand =
        LLVMGetOperand(currentStoreInstruction, 1);
...
void evaluateLoadInstructionForConstantReplacement(
    ...
    for (LLVMValueRef instruction = LLVMGetFirstInstruction(basicBlock);
        ...
        if (getOpcode(instruction) == LLVMStore) {
            ...
            inSetOfThisBlock =
                removeKilledStoreInstruction(inSetOfThisBlock, instruction);
        }

        if (getOpcode(instruction) == LLVMLoad) {
            replaceValidLoadInstructionsOperandsWithConstant(inSetOfThisBlock,
                                                             instruction);
        }
    }
}
...
set<LLVMValueRef> computeKillSet(LLVMBasicBlockRef basicBlock) {
    ...
    set<LLVMValueRef> allStoreInstructions =
        BLOCK_TO_ALL_STOREINSTRUCTIONS[basicBlock];
    ...
    for (auto &instruction : allStoreInstructions) {
        ...
        killSet.insert(allInstructionToThisMemory.begin(),
                       allInstructionToThisMemory.end());
    }
    return killSet;
}
    ...
    while (hasOutSetChanged) {
        ...
        for (LLVMBasicBlockRef basicBlock = LLVMGetFirstBasicBlock(function);
            ...
            for (const auto &predecessor : predecessors) {
                BLOCK_TO_INSET[basicBlock].insert(
                    BLOCK_TO_OUTSET[predecessor].begin(),
                    BLOCK_TO_OUTSET[predecessor].end());
            }
        }
        ...
        for (LLVMBasicBlockRef basicBlock = LLVMGetFirstBasicBlock(function);
            ...
            set_difference(inSet.begin(), inSet.end(),
                           KILL_SET[basicBlock].begin(),
                           KILL_SET[basicBlock].end(),
                           inserter(inMinusKillSets, inMinusKillSets.end()));

            // union
            set_union(GEN_SET[basicBlock].begin(), GEN_SET[basicBlock].end(),
                      inMinusKillSets.begin(), inMinusKillSets.end(),
                      inserter(currentOutSet, currentOutSet.end()));
...
bool walkBasicBlocksForGlobalOptimizations(LLVMValueRef function) {
    ...
    if (LOAD_INSTRUCTIONS_TO_REPLACE.size() > 0) {
        ...
        for (int index = 0; index < (int)LOAD_INSTRUCTIONS_TO_REPLACE.size();
             index++) {
...
bool walkFunctionsForGlobalOptimizations(LLVMModuleRef module) {
    ...
    for (LLVMValueRef function = LLVMGetFirstFunction(module); function;
        ...
        hasChanged =
            walkBasicBlocksForGlobalOptimizations(function) || hasChanged;
    }
    return hasChanged;
}
